...
/// of the wire object. No semantic validation happens on the .NET
...
public sealed class Profile
{
    ...
    /// <summary>On-wire nonce width in bits (<c>nonce_bits</c>), read
    /// from the blob's runtime globals. Inspection-only: non-null on a
    /// record from <see cref="Pipeline.Inspect"/>, <c>null</c> on one
    /// from <see cref="Pipeline.Lookup"/> and on one built by
    /// hand.</summary>
    public int? NonceBits { get; set; }

    /// <summary>DRBG barrier fill margin (<c>barrier_fill</c>), read
