...
//   C `void*`      -> `byte*`   (libitb3 buffer; always treated as bytes)
...
// libitb3 between the failing call and the diagnostic read may
...
{
    internal const string LibName = "libitb3";
...
/// <summary>
/// Resolves libitb3.{so,dylib,dll} at runtime and registers a
...
///      library file).
///   2. `&lt;repo&gt;/dist/&lt;os&gt;-&lt;arch&gt;/libitb3.&lt;ext&gt;` located by walking up
...
internal static class NativeLoader
{
    ...
    private static IntPtr Resolve(string libraryName, Assembly assembly, DllImportSearchPath? searchPath)
    {
        ...
        var env = Environment.GetEnvironmentVariable("ITB_LIBITB3_PATH");
        if (!string.IsNullOrEmpty(env) && File.Exists(env))
        {
            return LoadPreservingHostSignals(() => NativeLibrary.Load(env));
        }

        var dist = ResolveDistPath();
        if (dist is not null)
        {
            return LoadPreservingHostSignals(() => NativeLibrary.Load(dist));
        }

        return LoadPreservingHostSignals(
            () => NativeLibrary.Load(LibFilename, assembly, searchPath));
    }

    /// <summary>Loads the shared library and undoes the one signal-table
    /// change a Go c-shared runtime makes on the way in. See
    /// <see cref="SignalPolicy"/> for why this is necessary and for the
    /// limits of the correction.</summary>
    private static IntPtr LoadPreservingHostSignals(Func<IntPtr> load)
    {
        var before = SignalPolicy.SampleActivationFlags();
        var handle = load();
        SignalPolicy.RestoreActivationFlags(before);
        return handle;
    }
