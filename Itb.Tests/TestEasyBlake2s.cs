...
//
// Symmetric counterpart to bindings/python/tests/easy/test_blake2s.py
// applied to the high-level Encryptor surface. BLAKE2s ships only at
...
public class TestEasyBlake2s
{
    private static readonly (string Hash, int Width)[] Blake2sHashes = new[]
    {
        ("blake2s", 256),
    };
    ...
    public void RoundtripAcrossNonceSizes()
    {
        ...
        {
            foreach (var (hashName, _) in Blake2sHashes)
            {
    ...
    public void PersistenceAcrossNonceSizes()
    {
        ...
        foreach (var (hashName, width) in Blake2sHashes)
        {
    ...
    [Fact]
    public void HasPRFKeysIsTrueForBlake2s()
    {
        foreach (var (hashName, _) in Blake2sHashes)
        {
            using var enc = new Encryptor(hashName, 1024, "kmac256");
    ...
    public void SeedComponentsCountReflectsKeyBits()
    {
        foreach (var keyBits in new[] { 512, 1024, 2048 })
        {
            using var enc = new Encryptor("blake2s", keyBits, "kmac256");
    ...
    {
        using var enc = new Encryptor("blake2s", 1024, "kmac256");
    ...
    {
        using var src = new Encryptor("blake2s", 1024, "kmac256");
        var blob = src.Export();
