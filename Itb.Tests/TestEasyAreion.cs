...
//
// Symmetric counterpart to bindings/python/tests/easy/test_areion.py
// applied to the high-level Encryptor surface. Iterates both
// Areion-SoEM widths — areion256 (-256) and areion512 (-512) — across
// the three nonce sizes, both Ouroboros modes, and the persistence +
// plaintext-size sweeps.
//
// Persistence rides on Encryptor.Export / Import (JSON blob, single
// round-trip). The blob captures PRF keys, seed components, MAC key,
// and optional dedicated lockSeed material so the day-2 decrypt path
...
public class TestEasyAreion
{
    ...
    public void RoundtripAcrossNonceSizes()
    {
        ...
        {
            foreach (var (hashName, _) in AreionHashes)
            {
    ...
    public void PersistenceAcrossNonceSizes()
    {
        ...
        foreach (var (hashName, width) in AreionHashes)
        {
    ...
    [Fact]
    public void HasPRFKeysIsTrueForAreion()
    {
        foreach (var (hashName, _) in AreionHashes)
        {
            using var enc = new Encryptor(hashName, 1024, "kmac256");
    ...
    {
        using var enc = new Encryptor("areion256", 1024, "kmac256");
    ...
    {
        using var src = new Encryptor("areion256", 1024, "kmac256");
        var blob = src.Export();
    ...
    // are idempotent in the C# binding (handle becomes 0 after first
    // call).
    //
    // SKIPPED — test_context_manager: covered by `using var enc = ...`
    // at compile time.
    //
    // SKIPPED — test_bytearray_input / test_memoryview_input: covered
    // by ReadOnlySpan<byte> accepting any byte-slice reference.
    //
