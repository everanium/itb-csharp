// ChaCha20-focused C# binding coverage.
//
// Symmetric counterpart to TestBlake3.cs / TestBlake2s.cs: the same
// coverage shape (nonce-size sweep, single + Triple Ouroboros
// roundtrip, single + triple auth with tamper rejection, persistence
// sweep, plaintext-size sweep) applied to chacha20. ChaCha20 ships
// at a single width (-256) so this file iterates the single primitive
// across the same axes TestBlake3.cs / TestBlake2s.cs cover.
//
// Mirrors bindings/python/tests/test_chacha20.py one-to-one. Each
...
public sealed class TestChacha20
{
    /// <summary>Hash registry rows exercised by the test class —
    /// ChaCha20 ships only at -256.</summary>
    private static readonly (string Name, int Width)[] Hashes =
    {
        ("chacha20", 256),
    };

    /// <summary>Native fixed-key length per primitive — ChaCha20
    ...
    /// <summary>
    /// Single Ouroboros encrypt / decrypt over chacha20 × all three
    /// nonce sizes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Triple Ouroboros (7 seeds) encrypt / decrypt over chacha20 ×
    ...
    /// <summary>
    /// Single + Auth round trip + tamper rejection over chacha20 ×
    /// all three nonce sizes. Each MAC primitive is paired with
    /// chacha20 to confirm the auth scaffolding is width-agnostic.
    /// </summary>
    [Fact]
    ...
    /// chacha20 × all three nonce sizes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Encrypt with chacha20, snapshot (components + hash key), free
    ...
    /// <summary>
    /// Roundtrip chacha20 on plaintext sizes that span multiple chunk
    /// boundaries.
    /// </summary>
    [Fact]
