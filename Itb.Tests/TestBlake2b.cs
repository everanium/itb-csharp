// BLAKE2b-focused C# binding coverage.
//
// Symmetric counterpart to TestAreion.cs: the same coverage shape
// (nonce-size sweep, single + Triple Ouroboros roundtrip, single +
// triple auth with tamper rejection, persistence sweep, plaintext-
// size sweep) applied to blake2b256 / blake2b512.
//
// Mirrors bindings/python/tests/test_blake2b.py one-to-one. Each
...
public sealed class TestBlake2b
{
    /// <summary>Hash registry rows exercised by the test class —
    /// both BLAKE2b widths.</summary>
    ...
    /// <summary>Native fixed-key length per primitive — BLAKE2b256
    /// carries a 32-byte key, BLAKE2b512 carries a 64-byte key.</summary>
    ...
    /// <summary>
    /// Single Ouroboros encrypt / decrypt over blake2b256 + blake2b512
    ...
    /// <summary>
    /// Triple Ouroboros (7 seeds) encrypt / decrypt over blake2b256 +
    /// blake2b512 × all three nonce sizes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Single + Auth round trip + tamper rejection over blake2b256 +
    /// blake2b512 × all three nonce sizes. Each MAC primitive is
    /// paired with each BLAKE2b width to confirm the auth scaffolding
    /// is width-agnostic.
    /// </summary>
    [Fact]
    ...
    /// blake2b256 + blake2b512 × all three nonce sizes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Encrypt with blake2b, snapshot (components + hash key), free
    ...
    /// <summary>
    /// Roundtrip blake2b256 / blake2b512 on plaintext sizes that span
