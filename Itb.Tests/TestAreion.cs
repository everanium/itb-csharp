// Areion-SoEM-focused C# binding coverage.
//
// Symmetric counterpart to TestBlake2b.cs: the same coverage shape
// (nonce-size sweep, single + Triple Ouroboros roundtrip, single +
// triple auth with tamper rejection, persistence sweep, plaintext-
// size sweep) applied to areion256 / areion512.
//
// What this file adds beyond the cross-primitive iterators in
// TestNonceSizes / TestAuth (Agent B's scope):
//
//   * areion256 + areion512 coverage across all three NonceBits
//     values.
//   * areion256 + areion512 authenticated round-trip including
//     tamper rejection at the dynamic header offset.
//   * areion256 + areion512 persistence round-trip cycling
//     NonceBits across all three values inside a single test — the
//     Areion-SoEM ASM kernels are length-specialised at 20 / 36 /
//     68 byte buf shapes, so the full sweep surfaces any drift
//     between encrypt-time and decrypt-time NonceBits configuration.
//
// Mirrors bindings/python/tests/test_areion.py one-to-one. Each
...
public sealed class TestAreion
{
    /// <summary>Hash registry rows exercised by the test class —
    /// both Areion-SoEM widths.</summary>
    ...
    /// <summary>Native fixed-key length per primitive — Areion-SoEM-256
    /// carries a 32-byte fixed key, Areion-SoEM-512 carries a 64-byte
    /// fixed key.</summary>
    ...
    /// × all three nonce sizes. The four-pixel-batched VAES kernel
    /// runs a length-specialised path at 20 / 36 / 68 bytes; the loop
    /// below drives every shape through the FFI surface.
    /// </summary>
    [Fact]
    ...
    /// areion512 × all three nonce sizes. Each MAC primitive is
    /// paired with each Areion width to confirm the auth scaffolding
    /// is width-agnostic.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Encrypt with areion, snapshot (components + hash key), free
    /// the seeds, reconstruct via <see cref="Seed.FromComponents"/>,
    /// decrypt, verify the plaintext is bit-identical. Each
    /// (hash, key_bits, nonce_bits) combination must roundtrip — a
    /// regression in the Areion-SoEM ASM kernel that drifts between
    /// original and restored seeds (a mis-seeded fixedKey broadcast)
    /// would surface as a decrypt mismatch on the day-2 path while
    /// leaving the day-1 encrypt path silent.
    /// </summary>
    [Fact]
    ...
    /// unrecognised-hash-name negative paths. Locks per-primitive
    /// width and key-length contracts.
    /// </summary>
    [Fact]
    public void IntrospectionAndInvalidInputs()
    {
        ...
        foreach (var (hashName, width) in Hashes)
        {
            // Use the smallest key-bits multiple of the width.
            var keyBits = new[] { 512, 1024, 2048 }.First(k => k % width == 0);
            using var seed = new Seed(hashName, keyBits);
            ...
            Assert.Equal(keyBits / 64, comps.Length);
