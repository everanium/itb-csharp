// SipHash-2-4-focused C# binding coverage.
//
// W128 counterpart to TestAescmac.cs: the same coverage shape
// (nonce-size sweep, single + Triple Ouroboros roundtrip, single +
// triple auth with tamper rejection, persistence sweep, plaintext-
// size sweep) applied to siphash24. SipHash-2-4 ships at a single
// width (-128) so this file iterates the single primitive across
// the same axes TestAescmac.cs covers.
//
// SipHash-2-4 has no internal fixed key (its keying material is the
// seed components themselves), so the FFI hash-key field is empty
// — the persistence path round-trips through an empty span.
//
...
public sealed class TestSiphash24
{
    /// <summary>Hash registry rows exercised by the test class —
    /// SipHash-2-4 ships only at -128.</summary>
    private static readonly (string Name, int Width)[] Hashes =
    {
        ("siphash24", 128),
    };

    /// <summary>Native fixed-key length per primitive — SipHash-2-4
    /// has no internal fixed key (keyed by seed components), so the
    /// FFI hash-key field is empty.</summary>
    ...
    /// <summary>
    /// Single Ouroboros encrypt / decrypt over siphash24 × all three
    /// nonce sizes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Triple Ouroboros (7 seeds) encrypt / decrypt over siphash24 ×
    ...
    /// <summary>
    /// Single + Auth round trip + tamper rejection over siphash24 ×
    /// all three nonce sizes. Each MAC primitive is paired with
    /// siphash24 to confirm the auth scaffolding is width-agnostic.
    /// </summary>
    [Fact]
    ...
    /// siphash24 × all three nonce sizes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Encrypt with siphash24, snapshot (components + empty hash key),
    /// free the seeds, reconstruct via <see cref="Seed.FromComponents"/>,
    /// decrypt, verify the plaintext is bit-identical. SipHash-2-4
    /// round-trips through an empty hash-key span.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Roundtrip siphash24 on plaintext sizes that span multiple
    /// chunk boundaries.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Width / hash-name introspection plus the empty-key contract
    /// and the bad-keyBits / unrecognised-hash-name negative paths.
    /// </summary>
    [Fact]
    public void IntrospectionAndInvalidInputs()
    {
        ...
        foreach (var (hashName, width) in Hashes)
        {
            ...
            // SipHash-2-4 has no internal fixed key — empty array is
            // the canonical FFI response.
