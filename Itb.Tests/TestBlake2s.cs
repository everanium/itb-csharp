// BLAKE2s-focused C# binding coverage.
//
// Symmetric counterpart to TestBlake3.cs / TestChacha20.cs: the same
// coverage shape (nonce-size sweep, single + Triple Ouroboros
// roundtrip, single + triple auth with tamper rejection, persistence
// sweep, plaintext-size sweep) applied to blake2s. BLAKE2s ships at
// a single width (-256) so this file iterates the single primitive
// across the same axes TestBlake3.cs / TestChacha20.cs cover.
//
// Mirrors bindings/python/tests/test_blake2s.py one-to-one. Each
...
public sealed class TestBlake2s
{
    /// <summary>Hash registry rows exercised by the test class —
    /// BLAKE2s ships only at -256.</summary>
    private static readonly (string Name, int Width)[] Hashes =
    {
        ("blake2s", 256),
    };

    /// <summary>Native fixed-key length per primitive — BLAKE2s
    ...
    /// <summary>
    /// Single Ouroboros encrypt / decrypt over blake2s × all three
    /// nonce sizes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Triple Ouroboros (7 seeds) encrypt / decrypt over blake2s × all
    /// three nonce sizes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Single + Auth round trip + tamper rejection over blake2s × all
    /// three nonce sizes. Each MAC primitive is paired with blake2s
    ...
    /// <summary>
    /// Encrypt with blake2s, snapshot (components + hash key), free
    ...
    /// <summary>
    /// Roundtrip blake2s on plaintext sizes that span multiple chunk
    /// boundaries.
    /// </summary>
    [Fact]
